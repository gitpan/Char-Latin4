...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Elatin4::index($name, '::') == -1) && (Elatin4::index($name, "'") == -1)) {
            my $pkg;
...
# subroutines of package Elatin4
        ...
        elsif ($string =~ /\G \b Latin4::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin4::eval \b                                /oxgc) { $e_string .= 'eval Latin4::escape'; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Latin4::re_t=%s; while($Latin4::re_t =~ %s){%s local $^W=0; local $Latin4::re_r=%s; %s$Latin4::re_t="$`$Latin4::re_r$'"; pos($Latin4::re_t)=length "$`$Latin4::re_r"; } return $Latin4::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin4::re_r=CORE::eval $Latin4::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Latin4::re_r=%s; %s"%s$Latin4::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Latin4::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin4::re_r=%s; %s%s="$`$Latin4::re_r$'"; pos(%s)=length "$`$Latin4::re_r"; $Latin4::re_n++} return %s$Latin4::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Latin4::re_r=%s; %s%s="%s$Latin4::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Latin4::re_r=%s; %s%s="%s$Latin4::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin4::re_r=CORE::eval $Latin4::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
