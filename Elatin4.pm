...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin4::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin4::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin4::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin4::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin4::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin4::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Latin4.pm.
Because the Latin4.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin4::split returns
...
=item Chop string
  ...
  The Elatin4::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin4::index($string,$substr,$position);
  $pos = Elatin4::index($string,$substr);
  ...
  the Elatin4::index function returns -1.

=item Reverse index string

  $pos = Elatin4::rindex($string,$substr,$position);
  $pos = Elatin4::rindex($string,$substr);

  Works just like Elatin4::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin4::chr(65) is "A" in either ASCII or Latin-4, and
  Elatin4::chr(0x82a0) is a Latin-4 HIRAGANA LETTER A. For the reverse of Elatin4::chr,
  use Latin4::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin4::binmode(FILEHANDLE, $disciplines);
  Elatin4::binmode(FILEHANDLE);
  Elatin4::binmode($filehandle, $disciplines);
  Elatin4::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin4::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin4::open(FILEHANDLE, $expr);
  $rc = Elatin4::open(FILEHANDLE);
  $rc = Elatin4::open(my $filehandle, $mode, $expr);
  $rc = Elatin4::open(my $filehandle, $expr);
  $rc = Elatin4::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin4::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin4::open my $fh, ">logfile" or die ...
  ...
  Elatin4::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin4::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin4::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin4::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin4::open(FH, "<:crlf:encoding(Latin4)", $path) or die "can't open $path: $!";
