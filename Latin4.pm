...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin4::uc($char[$i]) ne Elatin4::lc($char[$i]))) {
            $char[$i] = '[' . Elatin4::uc($char[$i]) . Elatin4::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Latin4/Latin4/JA.pm
