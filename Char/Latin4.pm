...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin4::uc($char[$i]) ne Char::Elatin4::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin4::uc($char[$i]) . Char::Elatin4::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin4/Char::Latin4/JA.pm
...
Definitions in Char/Elatin4.pm.
  ...
  @{Char::Elatin4::eV}             qr{(?:[^\x0C\x0A\x0D])}
