...
# split
            ...
            my $e_string = 'Char::Elatin4::split';
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and (Char::Elatin4::uc($char[$i]) ne Char::Elatin4::lc($char[$i]))) {
...
and the cellular phone. This software treats Latin-4 directly. Therefor there is
not UTF8 flag.
...
=item * Possible to handle raw Latin-4 values

=item * Backward compatibility of data, script and how to

=item * Only Char/Latin4.pm and Char/Elatin4.pm, other modules are unnecessary
...
functions.
  ...
  ---------------------------------
  lc          Char::Elatin4::lc
  lcfirst     Char::Elatin4::lcfirst
  uc          Char::Elatin4::uc
  ucfirst     Char::Elatin4::ucfirst
  chr         Char::Elatin4::chr
  glob        Char::Elatin4::glob
  ---------------------------------
...
=head1 Perl5.6 Emulation on perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Elatin4.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Elatin4::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Elatin4::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005, Char::Latin4 software emulate perl5.6's binmode function.
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  open(FH, "<:crlf:encoding(Char::Latin4)", $path) or die "can't open $path: $!";
...
=head1 Ignore utf8 pragma
  ...
  ---------------------------------------------------------------------
  use utf8;       # use utf8;            Char/Elatin4.pm provides utf8::*
  no utf8;        # no utf8;             functions even if 'no utf8;'
  use bytes;      # use bytes;           Char/Elatin4.pm provides bytes::*
...
page 402 of Programming Perl, 3rd ed. again.
  ...
  Information processing model beginning with this software
    ...
    +-----------------------------------+
    |      ASCII Compatible Encoding    | ex. Latin-4
    +-----------------------------------+
                (No UTF8 Flag)
...
I am thankful to all persons.
 ...
 http://search.cpan.org/dist/Latin-4-Regexp/
