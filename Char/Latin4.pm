...
######################################################################
#
# Char::Latin4 - Source code filter to escape Latin-4 script
#
# http://search.cpan.org/dist/Char-Char::Latin4/
#
...
BEGIN { CORE::require Char::Elatin4; }
...
my $function_ord;         # ord()   to ord()   or Char::Latin4::ord()
my $function_ord_;        # ord     to ord     or Char::Latin4::ord_
my $function_reverse;     # reverse to reverse or Char::Latin4::reverse
my $function_getc;        # getc    to getc    or Char::Latin4::getc
...
END
    ...
    print Char::Latin4::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Elatin4::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Latin4::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Elatin4::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Latin4::escape_script {
    ...
    # read Latin-4 script
    my $fh = gensym();
    Char::Elatin4::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Elatin4(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Elatin4 %s;\n", $Char::Latin4::VERSION); # require run-time routines version

        # use Char::Latin4 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Latin4 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Latin4::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Latin4.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Latin4.pm $version, but @{[__FILE__]} is version $Char::Latin4::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Elatin4.pm $version, but \\\$Char::Elatin4::VERSION is \$Char::Elatin4::VERSION" if \$Char::Elatin4::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Latin4::VERSION) {
                    die "Script $0 required Char/Latin4.pm $version, but @{[__FILE__]} is only version $Char::Latin4::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Latin4::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Latin4::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Latin4::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Latin4::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Elatin4
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Latin4::re_t=%s; while($Char::Latin4::re_t =~ %s){%s local $^W=0; local $Char::Latin4::re_r=%s; %s$Char::Latin4::re_t="$`$Char::Latin4::re_r$'"; pos($Char::Latin4::re_t)=length "$`$Char::Latin4::re_r"; } return $Char::Latin4::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin4::re_r=eval $Char::Latin4::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Latin4::re_r=%s; %s"%s$Char::Latin4::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Latin4::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin4::re_r=%s; %s%s="$`$Char::Latin4::re_r$'"; pos(%s)=length "$`$Char::Latin4::re_r"; $Char::Latin4::re_n++} return %s$Char::Latin4::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Latin4::re_r=%s; %s%s="%s$Char::Latin4::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Latin4::re_r=%s; %s%s="%s$Char::Latin4::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin4::re_r=eval $Char::Latin4::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Latin4.pm and Char/Elatin4.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Latin4;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Latin4;
  use Char::Latin4 ver.sion;             --- require minimum version
  use Char::Latin4 ver.sion.0;           --- expects version (match or die)

  # "no Char::Latin4;" not supported

  or

  $ perl Char/Latin4.pm Latin-4_script.pl > Escaped_script.pl.e
...
Char::Latin4 software is "middleware" between perl interpreter and your Perl script
written in Latin-4.
...
By "use Char::Latin4;", it automatically interpret your script as Latin-4. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Latin4.pm, Char/Elatin4.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Elatin4.pm
works well.
...
=head1 Software Composition
   ...
   Char/Latin4.pm               --- source code filter to escape Latin-4
   Char/Elatin4.pm              --- run-time routines for Char/Latin4.pm
...
You need write 'use Char::Latin4;' in your script.
  ...
  Before      You do
  ---------------------------------
  (nothing)   use Char::Latin4;
  ---------------------------------

=head1 Calling 'Char::Elatin4::ignorecase()' (Char/Latin4.pm provides)

Char/Latin4.pm applies calling 'Char::Elatin4::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Elatin4::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Elatin4.pm provides)
...
Definitions in Char/Elatin4.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Latin4.pm provides)

Char/Latin4.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Latin4.pm and Char/Elatin4.pm provide)

Insert 'Char::Elatin4::' at head of function name. Char/Elatin4.pm provides your script Char::Elatin4::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Elatin4::lc        Character
  lcfirst     Char::Elatin4::lcfirst   Character
  uc          Char::Elatin4::uc        Character
  ucfirst     Char::Elatin4::ucfirst   Character
  fc          Char::Elatin4::fc        Character
  chr         Char::Elatin4::chr       Character
  glob        Char::Elatin4::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Latin4.pm provides)

Char/Latin4.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Elatin4.pm provides)

Char/Elatin4.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Latin4' }" at head.
Store the standard module modified for Char::Latin4 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Latin4 and change
'use utf8;' to 'use Char::Latin4;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Elatin4.pm provides these
  functions.
...
Char::Latin4 software remains one language and one interpreter.
